            ...
            <form method="POST" action="{{ route('profiles.update', $profile) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="email" class="form-control" id="email" name="email"
                        placeholder="name@example.com" value="{{ old('email', $profile->email) }}">
                    @error('email')
                        <p class="text-danger mb-2">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="flex">
                        <img src="{{ asset('../storage/' . $profile->image) }}" style="width:35px; height:35px; object-fit:cover; border-radius:50%;"/>
                        <input type="file" name="image" id="image" class="form-control mx-2">
                    </div>
                    @error('image')
                        <p class="text-danger mb-2">{{ $message }}</p>
                    @enderror
